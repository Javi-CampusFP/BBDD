...
-- Ejercicio 17
CREATE OR REPLACE PROCEDURE encuestas.crear_encuestas()
LANGUAGE plpgsql
AS $body$
DECLARE
...
-- Borrar la tabla encuestas si ya existe.
	DROP TABLE IF EXISTS encuestas.resultado;
...
valoraciones[3] as nota_usabilidad, ((valoraciones[1] + valoraciones[2] + valoraciones[3]) / 3.0) AS media_nota 
...
((valoraciones[1] + valoraciones[2] + valoraciones[3]) / 3.0) as media_nota 
...
AVG(valoraciones[1]) AS media_sonido, AVG(valoraciones[2]) AS media_imagen, 
AVG(valoraciones[3]) AS media_usabilidad 
FROM encuestas.resultado 
...
AVG(valoraciones[1]) AS MEDIA_SONIDO, AVG(valoraciones[2]) AS MEDIA_IMAGEN, 
AVG(valoraciones[3]) AS MEDIA_USABILIDAD, (AVG(valoraciones[1] + valoraciones[2] + valoraciones[3]) / 3.0) as MEDIA_GENERAL 
FROM encuestas.resultado 
INNER JOIN encuestas.provincia ON provincia.codigo = codigoprovincia 
GROUP BY resultado.codigoprovincia, provincia.nombre LIMIT 20;
...
HAVING ((AVG(valoraciones[1] + valoraciones[2] + valoraciones[3]) / 3.0) <= 6) LIMIT 1;
...
INNER JOIN encuestas.provincia ON provincia.codigo = codigoprovincia 
GROUP BY resultado.codigoprovincia, provincia.nombre 
ORDER BY MEDIA_GENERAL DESC LIMIT 1;
...
FROM encuestas.provincia INNER JOIN encuestas.resultado 
...
-- Ejercicio 25
-- Crear la tabla
CREATE TABLE IF NOT EXISTS encuestas.encuestas_old(
  	idencuesta serial PRIMARY KEY,
	codigoprovincia character(2),
 	valoraciones INT[3]
);

-- Crear la función que va a ejecutar el trigger
...
CREATE TRIGGER mover_datos
AFTER DELETE
ON encuestas.resultado
FOR EACH ROW
EXECUTE FUNCTION encuestas.mover_datos();

-- Elimina alguna encuesta para comprobar el funcionamiento
DELETE FROM encuestas.resultado WHERE idencuesta = 1;
SELECT * FROM encuestas.encuestas_old;

-- Ejercicio 26
SELECT provincia.codigo AS CODPROV, 
	provincia.nombre AS NOMBRE_PROVINCIA,
	SUM(valoraciones[1] / 10) AS MEDIA_SONIDO,
	SUM(valoraciones[2] / 10) AS MEDIA_IMAGEN,
	SUM(valoraciones[3] / 10) AS MEDIA_USABILIDAD,
	SUM(valoraciones[1] + valoraciones[2] + valoraciones[3]) / 30.0 AS MEDIA_GENERAL
FROM encuestas.provincia
INNER JOIN encuestas.resultado
